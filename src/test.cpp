...
static int map_test(int argc, char *argv[], bool all) {
	...
	map<testdata, int>::iterator it = a.begin();
	for (; it != a.end(); it = a.next(it)) {
...
struct accept_watcher {
	server &m_s;
	DSCRPTR m_fd;
	accept_watcher(server &s) : m_s(s), m_fd(INVALID_FD) {}
	int operator () (DSCRPTR fd, connect_handler &ch) {
		m_fd = fd;
		return m_s.spool()(fd, ch);
	}
	session *attached() { return m_fd == INVALID_FD ? NULL : m_s.served_for(m_fd); }
};
...
static bool session_test3(int argc, char *argv[], bool all) {
	...
	verify_success(s.init());
	accept_watcher aw(s);
	accept_handler ah(aw);
	s.listen("tcp://0.0.0.0:8888", ah);
	for (int i = 0; i < WATCHERS; i++) {
		_sw[i].n_id = (i + 1);
		_sw[i].sv = &s;
		session::watcher sw(_sw[i]);
