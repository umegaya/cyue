return function ()
	local y = yue or require('yue')
	local ffi = (function()
		local ok,r = pcall(require,'ffi')
		return ok and r or nil
	end)()
	local C = ffi and ffi.C or nil
	-- hide yue from global namespace
	yue = nil
	return {
	-- ===========================================================================
	-- @module_name:	yue.ffi
		...
		m.sht = (function ()
			return setmetatable({}, {
				__newindex = function (tbl, key, val)
					...
					local cdecl = string.format('typedef struct { %s } %s;', vars, name) 
					yue.ffi.cdef(cdecl)
