...
//	lua_pushcfunction(vm, future::gc);
	...
	case LUA_TTABLE: /* = map {...} */
		if ((r = call_custom_pack(vm, sr, stkid)) < 0) {
			if (r == NBR_ENOTFOUND) {
				...
				if (pack_table(vm, sr, stkid) < 0) { return NBR_ELENGTH; }
				break;
			}
			return r;
		} break;
...
lua_State *yue_newthread(yue_ThreadCB cb) {
